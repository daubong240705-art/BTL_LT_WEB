import { Category, Episode, Movie } from "@/app/types/movie.type";
...
export const getMovieEpisode = async (id: number): Promise<Episode[]> => {
    const res = await fetch(`http://localhost:8080/api/v1/episodes/movie/${id}`, {
        next: { revalidate: 3600 },
    });
