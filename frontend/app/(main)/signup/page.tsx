...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: SignupForm) => {
    ...
    if (res?.statusCode === 200 || res?.statusCode === 201) {
      toast.success("Tạo tài khoản thành công đăng nhập để tiếp tục")
      router.push("/login")
      return
    }
    ...
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden py-12 bg-black">
      ...
      <div className="relative z-10 w-full max-w-md px-6 ">
        <div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-2xl shadow-2xl p-8">
          ...
          <div
            className="w-full h-4 mt-6 flex items-center justify-center border-zinc-700 bg-transparent border text-zinc-300 hover:bg-zinc-800 hover:text-white py-6 rounded-lg transition-all">
            <Link
              href="/"
            >
